...
(async () => {
    try  {
        ...
        audioClassifier.on('result', (ev, timeMs) => {
            ...
            var valuableWakewords = ["excuseme", "qingwen"]; // wake words; adjust as needed
            var recordingInProgress = false;

            for (let k of Object.keys(c)) {
                c[k] = (<number>c[k]).toFixed(4); // fixed precision
                if (!recordingInProgress && c[k] > 0.7 && valuableWakewords.includes(k)) {
                    // A wake word has been said
                    recordingInProgress = true;
                    // Start recording audio for 10 seconds
                    var file = fs.createWriteStream('question_' + recordingCount + '.wav', { encoding: 'binary' });
                    recordingCount++;
